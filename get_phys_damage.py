...
@njit
def get_phys_damage(wpn_dmg, fstr_wpn, wsc, pdif, ftp, crit, crit_dmg, wsd, ws_bonus, ws_trait, n, sneak_attack_bonus=0, trick_attack_bonus=0):
    #
...
@njit
def get_avg_phys_damage(wpn_dmg, fstr_wpn, wsc, pdif, ftp, crit_rate, crit_dmg, wsd, ws_bonus, ws_trait, sneak_attack_bonus=0, trick_attack_bonus=0):
    #
    # Calculate average physical damage dealt for a single attack of a weapon skill. Uses equation from BG wiki
    # https://www.bg-wiki.com/ffxi/Weapon_Skill_Damage
    #
    phys = int(  ((wpn_dmg + fstr_wpn + wsc)*ftp + sneak_attack_bonus + trick_attack_bonus) * pdif * (1 + min(crit_rate,1.0)*min(crit_dmg,1.0)) * (1+wsd)*(1+ws_bonus)*(1+ws_trait)  ) # crit_rate is a number between 0 and 1
    return(phys)
