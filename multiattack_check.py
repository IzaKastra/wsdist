...
@njit
def multiattack_check(bonus_hits, ma_tp_hits, total_hits, player_attack, wpn_type_skill, pdl_trait, pdl_gear, enemy_defense, crit_rate, wpn_dmg, fstr, wsc, ftp2, crit_dmg, ws_bonus, ws_trait, hitrate):
    physical_damage = 0
    for k in range(bonus_hits):
        ...
        if random.uniform(0,1) < hitrate:
            ...
            physical_damage += get_phys_damage(wpn_dmg, fstr, wsc, pdif, ftp2, crit, crit_dmg, 0, ws_bonus, ws_trait, 1) # Notice that multi-attacks all use wsd=0 and ftp2
